...
def run_trial_render(m, policy, encoder, adapt, cfg, cond, mode,
    ...
    d = mujoco.MjData(m)
    no_jitter = cond.seed < 0
    rng = np.random.default_rng(0 if no_jitter else cond.seed)
    ...
    jitter = (np.zeros(n_leg, dtype=np.float32) if no_jitter
              else rng.normal(0, 0.02, size=n_leg).astype(np.float32))
